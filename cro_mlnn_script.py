...
def train_model(item):
    ...
    paras_name = "hs_{}-ep_{}-act_{}-ps_{}-po_{}-Fb_{}-Fa_{}-Fd_{}-Pd_{}-G_{}-GCR_{}-k_{}".format(item["hidden_size"], item["epoch"], item["activations"],
                        item["pop_size"], item["po"], item["Fb"], item["Fa"], item["Fd"], item["Pd"], item["G"], item["GCR"], item["k"])
    root_hybrid_paras = {
    ...
    cro_paras = {
        ...
        "Fd": item["Fd"], "Pd": item["Pd"], "G": item["G"], "GCR": item["GCR"], "k": item["k"]
    }
    md = CroMlnn(root_base_paras=root_base_paras, root_hybrid_paras=root_hybrid_paras, cro_paras=cro_paras)
    md._running__()
