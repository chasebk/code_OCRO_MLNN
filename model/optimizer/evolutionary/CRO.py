...
class BaseCRO(RootAlgo):
    ...
    def _sort_occupied_position__(self):
        def referHealth(location):
            return self.reef[location]['health'][self.ID_ERROR]
    ...
    def _larvae_setting__(self, larvae):
        for larva in larvae:
            ...
            for i in range(self.k):
                ...
                if self.reef[p]['occupied'] == 0 or self.reef[p]['health'][self.ID_ERROR] > larva_fit[self.ID_ERROR]:
    ...
    def _budding_setting__(self, larvae):
        for larva in larvae:
            for i in range(self.k):
                p = random.randint(0, self.pop_size - 1)
                if self.reef[p]['occupied'] == 0 or (self.reef[p]['health'][self.ID_ERROR] > larva['health'][self.ID_ERROR]) :
    ...
    def _train__(self):
        ...
        for j in range(0, self.epoch):
            ...
            if bes_sol['health'][self.ID_ERROR] < best_train["health"][self.ID_ERROR]:
                best_train = bes_sol
            if self.print_train:
                print("> Epoch {}: Best training fitness {}".format(j + 1, best_train["health"][self.ID_ERROR]))
...
class OCRO(BaseCRO):
    ...
    def __opposition_based_larva__(self, budding_pos):
        ...
        for i in budding_pos:
            ...
            if Olarva['health'][self.ID_ERROR] < self.reef[i]['health'][self.ID_ERROR]:
