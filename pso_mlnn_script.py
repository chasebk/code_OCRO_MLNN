...
from model.main.hybrid_mlnn import PsoMlnn
from utils.SettingPaper import pso_mlnn_paras_final as param_grid
...
def train_model(item):
    ...
    paras_name = "hs_{}-ep_{}-act_{}-ps_{}-pc_{}-pm_{}".format(item["hidden_size"], item["epoch"], item["activation"], item["pop_size"], item["c_minmax"],
                                                               item["w_minmax"])
    root_hybrid_paras = {
    ...
    pso_paras = {
        "epoch": item["epoch"], "pop_size": item["pop_size"], "c_minmax": item["c_minmax"], "w_minmax": item["w_minmax"]
    }
    md = PsoMlnn(root_base_paras=root_base_paras, root_hybrid_paras=root_hybrid_paras, pso_paras=pso_paras)
    md._running__()
